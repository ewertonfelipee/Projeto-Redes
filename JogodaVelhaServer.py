...
def clicado (btn, i, j):
    ...
    if (btn["text"] == " " and meu_turno == 1):
        ...
        button_number = i*3+j
        cliente.send(str(button_number).encode('utf-8'))
        meu_turno=0
        verificar(btn)
...
def recvThread (cliente):
    global btns
    global meu_turno
    while True:
        button_number = int(cliente.recv(2).decode('utf-8'))
...
start_new_thread(recvThread, (cliente, ))
